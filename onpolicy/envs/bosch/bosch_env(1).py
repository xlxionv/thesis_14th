...
class BoschEnv(object):
    ...
    def _heuristic_allocate(self, masks):
        """
        Given binary masks (L x P), fill queues using demand-proportional
        allocation.

        Two-stage logic:
          - URGENCY (full lookahead window): determines which products to
            prioritize and whether pre-building is warranted.
          - QUANTITY (single-period cap): limits how much to actually queue
            so we don't overshoot inventory.  The quantity target per product
            is: today's demand + backlog - inventory - queue,
            ensuring only ~1 day of production is queued at a time.
        """
        ...
        # --- URGENCY & QUANTITY (Look across the full 5-day horizon) ---
        lookahead_end = min(t + self.allocator_lookahead, self.num_periods)
        if lookahead_end > t:
            future_demand = np.sum(self.demand[t:lookahead_end], axis=0).astype(np.float32)
        else:
            future_demand = np.zeros(P, dtype=np.float32)

        # Total amount needed in the next 5 days
        total_qty_target = (
            future_demand
            + self.backlog
            - self.inventory
        ...
        # Urgency is used to sort which product gets priority on the machine
        urgency = total_qty_target.copy()

        # FIX: track the last allocated product per line within this allocation
        # call so that setup time for the second+ product on a line correctly
        # uses the previous allocated product (not the previous period's product).
        last_allocated = self.line_setup.copy()  # start from previous period state

        # For each line, allocate activated products
        for l in range(L):
            remaining_cap = float(self.capacity_per_line[l])

            # Get activated products, sorted by URGENCY
            active = []
            for p in range(P):
                if masks[l, p] > 0.5:
                    active.append((p, float(urgency[p])))
            active.sort(key=lambda x: -x[1])  # most urgent first

            for p, _urg in active:
                ...
                # FIX: estimate setup time using last_allocated[l], which is
                # updated as products are scheduled on this line, so the second
                # product's setup is from the first product → second, not from
                # previous period → second.
                ...
                cap_after_setup = max(0.0, remaining_cap - setup_time)

                # The machine can only process a maximum of 1 day's worth of capacity.
                max_units_today = cap_after_setup / proc

                # We queue the smaller of: (What we need for 5 days) OR (What the machine can actually do today)
                ...
                total_qty_target[p] -= qty   # reduce for other lines
                urgency[p] -= qty            # keep urgency consistent
                last_allocated[l] = p        # FIX: update for next product on same line
